...
data <- read.csv('Shih.csv', header = TRUE)
...
for (i in 1:3){
  ...
  plots_right_side(xnew, compVal = 5, ropeRad = 0.0, plotname[i], HDImass = .95)}
  else{
    plotname[i] <- paste0("Phase-", i, ":Phase-", (i + 1))
    xnew <- x[((x[,i]>quantile(x[,i], .02)) & (x[,i]<quantile(x[,i],.98))),i]
    plots_left_side(xnew, compVal = 1/5, ropeRad = 0.0, plotname[i], HDImass = .95)
  }
}
...
#to plot the data
data1 <- data[, 1:2]
a <- c(data1[data1[, 1]==1, 2], rep(NA, 11))
b <- data1[data1[, 1]==2, 2]
c <- c(data1[data1[, 1]==3, 2], rep(NA, 12))
d <- c(data1[data1[, 1]==4, 2], NA)
plot.data <- rbind(a, b, c, d)
plot_SSD(plot.data)
data1 <- cbind(data1, c(1:48))
